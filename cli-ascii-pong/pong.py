...
def start_screen(stdscr):
    ...
    while True:
        ...
        if max_y < required_y or max_x < required_x:
            stdscr.addstr(
                0,
                0,
                f"Terminal too small! Resize to at least {required_x}x{required_y}.",
            )
            stdscr.refresh()
            time.sleep(1)
            continue
    ...
    return (
        PLAYER_TYPES[player_keys[left_selection]],
...
def render_game(stdscr, state):
    ...
    for y in range(1, FIELD_HEIGHT + 1):
        ...
        for x in range(1, FIELD_WIDTH + 1):
            if (
                x == 2
                and state.left_player.paddle_y
                <= y
                < state.left_player.paddle_y + state.left_player.height
            ):
                line += state.left_player.characters[y - state.left_player.paddle_y]
            elif (
                x == FIELD_WIDTH - 1
                and state.right_player.paddle_y
                <= y
                < state.right_player.paddle_y + state.right_player.height
            ):
    ...
    while True:
        ...
        if max_y < FIELD_HEIGHT + 4 or max_x < FIELD_WIDTH + 2:
            ...
            stdscr.addstr(
                0,
                0,
                f"Terminal too small! Resize to at least {FIELD_WIDTH + 2}x{FIELD_HEIGHT + 4}.",
            )
            stdscr.refresh()
            time.sleep(1)
            continue
        ...
        stdscr.addstr(
            FIELD_HEIGHT + 3,
            0,
            "Press 'q' to quit. W/S: left paddle, Up/Down: right paddle",
        )
        stdscr.nodelay(True)
        key = stdscr.getch()
        if key == ord("q"):
